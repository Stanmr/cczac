...
if(!function_exists('swm_woocommerce_catalog_ordering')) {
	function swm_woocommerce_catalog_ordering() {	
		...
		$product_order['default'] 		= esc_html__("Default Order",'election');
		$product_order['name'] 			= esc_html__("Name",'election');
		$product_order['price'] 		= esc_html__("Price",'election');
		$product_order['date'] 			= esc_html__("Date",'election');
		$product_order['rating'] 		= esc_html__("Rating",'election');
		$product_order['popularity'] 	= esc_html__("Popularity",'election');
		$product_sort['asc'] 			= esc_html__("Sort Ascending",  'election');
		$product_sort['desc'] 			= esc_html__("Sort Descending",  'election');
		$swm_p_items_per_page 				= esc_html__("Products",'election');
		...
		$output .= '<li>';	
		$output .= "<span class='current-select'>".esc_html__('Sort by','election')." ".ucfirst($swm_pob)." </span>";
		$output .= '<ul>';
		...
		$output .= '<li>';		
		$output .= "<span class='current-select'>".esc_html__('Show','election')." ".$swm_pc." ".$swm_p_items_per_page."</span>";
		$output .= '<ul>';
...
if(!function_exists('my_woocommerce_breadcrumbs')) {
	function my_woocommerce_breadcrumbs() {
	    return array(
	            ...
	            'home'        => _x( 'Home', 'breadcrumb', 'election' ),
	        );
	}
} 
...
if(!function_exists('woocommerce_pagination')) {
	function woocommerce_pagination() {
		...
		global $wp_query;
		$output = '';
	   	$prevPage = esc_html__( 'Prev', 'election' );
		$nextPage = esc_html__( 'Next', 'election' );
...
if(!function_exists('swm_woocommerce_single_product_image_thumbnail_html')) {
	function swm_woocommerce_single_product_image_thumbnail_html( $html, $attachment_id ) {
	    $single_page_gallery_thumbnails = (get_option('swm_woo_single_page_gallery_thumbnails') <> '') ? esc_attr(get_option('swm_woo_single_page_gallery_thumbnails')) : 5;
...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) { 
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		ob_start();	
		?>
		<a class="cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php echo esc_html__('View your shopping cart', 'election'); ?>"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'election'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
		<?php	
		...
		<div id="widget_search_form">
			<form role="search" method="get" id="searchform" action="' . esc_url( home_url( '/' ) ) . '">
				<div>		
					...
					<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . esc_html__( 'Search Products', 'election' ) . '" />
