...
function swm_breadcrumb_trail_get_items( $args = array() ) {
	...
	elseif ( is_singular() ) {
		...
		if ( class_exists('Woocommerce') ) {
			...
			$shop_title = esc_html__('Shop','election');
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( esc_html__( 'g:i a', 'election' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'election' ), get_the_time( esc_html__( 'i', 'election' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( esc_html__( 'g a', 'election' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'election' ) ) . '">' . get_the_time( esc_html__( 'Y', 'election' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'election' ) ) . '">' . get_the_time( esc_html__( 'F', 'election' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( esc_html__( 'd', 'election' ) );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'election' ) ) . '">' . get_the_time( esc_html__( 'Y', 'election' ) ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'election' ), get_the_time( esc_attr__( 'W', 'election' ) ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'election' ) ) . '">' . get_the_time( esc_html__( 'Y', 'election' ) ) . '</a>';
				$trail['trail_end'] = get_the_time( esc_html__( 'F', 'election' ) );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( esc_html__( 'Y', 'election' ) );
			}
		}
	}
	...
	elseif ( is_search() ) {		
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'election' ), esc_attr( get_search_query() ) );
	}

	/* If viewing a 404 error page. */
	elseif ( is_404() ) {
		$trail['trail_end'] = esc_html__( '404 Not Found', 'election' );

	/* If viewing event calendar pages. */
	} elseif ( class_exists( 'Tribe__Events__Main' ) ) {
        if ( tribe_is_upcoming()) {
            $trail['trail_end'] = esc_html__( 'Events', 'election' );              
...
function swm_breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'election' ), $post_id ) . '">' . get_the_time( esc_html__( 'Y', 'election' ), $post_id ) . '</a>';

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'election' ), $post_id ) . '">' . get_the_time( esc_html__( 'F', 'election' ), $post_id ) . '</a>';

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'election' ), $post_id ) . '">' . get_the_time( esc_html__( 'd', 'election' ), $post_id ) . '</a>';
...
function swm_breadcrumb_trail_textdomain() {
	return apply_filters( 'swm_breadcrumb_trail_textdomain', 'election' );
}
